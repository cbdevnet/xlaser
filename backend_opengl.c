...
int backend_init(XRESOURCES* res, CONFIG* config){
	...
	glGenTextures(2, res->backend.fbo_texture);
	glGenFramebuffers(2, res->backend.fboID);
	glGenRenderbuffers(2, res->backend.rbo_depth);
	...
	res->backend.program[1].modelview = glGetUniformLocation(res->backend.program[1].id, "goboSize");
	...
	res->backend.program[2].modelview = glGetUniformLocation(res->backend.program[2].id, "modelview");
	return 0;
}
...
int xlaser_render(XRESOURCES* xres, uint8_t* channels){
	...
	double window_x_scale = scaling_factor * ((win_scale <= 1.0) ? win_scale : 1.0);
	double window_y_scale = scaling_factor / ((win_scale > 1.0) ? win_scale : 1.0);
	...
	//actually draw
	if(xres->backend.update_last_frame){
		glViewport(0, 0, xres->window_width * 1.2, xres->window_height * 1.2);
		...
		glUniform2f(xres->backend.program[1].modelview, window_x_scale * xres->window_width, window_y_scale * xres->window_height);
